...
function carregarDashboard() {
    ...
    document.getElementById('userNicknameMini').textContent = usuarioAtual.nickname;
    
    // Definir avatar do usuário
    const avatarMini = document.getElementById('userAvatarMini');
    if (avatarMini && usuarioAtual.foto) {
        if (avatarMini.tagName === 'IMG') {
            avatarMini.src = usuarioAtual.foto;
        } else {
            avatarMini.style.backgroundImage = `url('${usuarioAtual.foto}')`;
    ...
    document.getElementById('playersOnline').textContent = jogadoresSimulados.filter(j => j.online).length;
    document.getElementById('myMatches').textContent = matchesSimulados.length;
    document.getElementById('savedGames').textContent = usuarioAtual.jogos;
    document.getElementById('badgeMatches').textContent = matchesSimulados.filter(m => m.status === 'pendente').length;
...
function carregarJogosDestaque() {
    ...
    container.innerHTML = jogosSimulados.slice(0, 6).map(jogo => `
...
function carregarJogadoresRecentes() {
    ...
    container.innerHTML = jogadoresSimulados.slice(0, 4).map(jogador => `
...
function carregarMatchesRecentes() {
    ...
    container.innerHTML = matchesSimulados.slice(0, 3).map(match => {
