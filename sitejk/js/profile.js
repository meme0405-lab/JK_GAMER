...
function carregarPerfil() {
    ...
    document.getElementById('profileNickname').textContent = `@${usuarioAtual.nickname}`;
    ...
    // Stats
    document.getElementById('statMatches').textContent = usuarioAtual.matches;
    document.getElementById('statGames').textContent = usuarioAtual.jogos;
    document.getElementById('statLevel').textContent = usuarioAtual.nivel;
    ...
    document.getElementById('profileSince').textContent = formatarData(new Date(2023, 0, 15));
...
function carregarJogosFavoritos() {
    ...
    // Remover animação de loading quando jogos forem carregados
...
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('profileName')) {
        ...
        // Remover animação de loading após carregar dados
        setTimeout(() => {
            const loader = document.querySelector('.loading-spinner');
            if (loader) loader.style.display = 'none';
        }, 800);
